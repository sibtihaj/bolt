...
//
// Groups are split so group-level WithHideFunc can skip conditional sections:
//   1. Basic info (name, cluster type, namespace, hostname, mode, TLS)
//   2–5. One group per cloud provider (EKS / AKS / GKE / kubeadm)
//   6. Core credentials (license, encryption password)
//   7. TLS cert+key paths (hidden when generate-TLS is chosen)
//   8. External storage (hidden for disk mode)
//   9. Redis URL (hidden unless active-active)
...
func interactiveDeployK8s() error {
	...
	err := huh.NewForm(
		...
		).WithHideFunc(func() bool { return clusterType != "eks" }),
		...
		).WithHideFunc(func() bool { return clusterType != "aks" }),
		...
		).WithHideFunc(func() bool { return clusterType != "gke" }),
		...
		// ── 7. TLS cert+key (skipped when self-signed TLS is chosen) ──────────
		huh.NewGroup(
			huh.NewInput().
		...
		// ── 8. External storage (skipped for disk mode) ───────────────────────
		huh.NewGroup(
			huh.NewInput().
	...
	// Resolve TLS paths for self-signed mode
	if generateTLS {
	...
	// For cloud providers, leave Kubeconfig empty so the provisioner runs
	// the cloud-specific kubeconfig setup. For kubeadm, use the provided path.
	resolvedKubeconfig := ""
