...
func (p *K8sProvisioner) Deploy(creds *credentials.TFECredentials) error {
	...
	if d.Kubeconfig == "" {
		...
		if err := p.configureKubeconfig(creds); err != nil {
	...
	if err := helm.RepoAdd(d); err != nil {
		return err
	}
	if err := helm.RepoUpdate(d); err != nil {
		return err
	}

	// 7. Helm install (streams live output)
	fmt.Println("→ Installing Terraform Enterprise via Helm (this may take several minutes)...")
	timeout := "10m"
	if err := helm.Install(d, valuesPath, timeout); err != nil {
...
func (p *K8sProvisioner) configureKubeconfig(creds *credentials.TFECredentials) error {
	...
	case state.ClusterKubeadm:
		// kubeadm: default kubeconfig is used as-is
		return nil
	default:
